...
- [ ] **DEPLOY-06**: `prisma migrate deploy`로 데이터베이스 마이그레이션을 자동화할 수 있다
...
- [ ] **PERF-01**: Prisma Singleton 패턴으로 연결 풀링을 구현할 수 있다 (connection_limit=10)
- [ ] **PERF-02**: Prisma `include`로 N+1 쿼리 문제를 해결할 수 있다
- [ ] **PERF-03**: 자주 조회하는 필드에 데이터베이스 인덱스를 생성할 수 있다
- [ ] **PERF-04**: Next.js `<Image>` 컴포넌트로 이미지 최적화를 적용할 수 있다 (WebP/AVIF 변환)
