...
<objective>
PDF 생성 Server Actions 및 API 라우트 구현 - 비동기 생성, 캐싱, 상태 조회

Purpose: PDF 생성을 위한 서버 사이드 로직을 구현합니다. Phase 6의 after() 패턴을 활용하여 비동기 생성하고, 데이터베이스 캐싱으로 중복 생성을 방지합니다.
Output: PDF 생성/조회 Server Actions, PDF 스트리밍 API, ReportPDF DAL 함수
</objective>
...
4. generateConsultationReport uses after() for async generation
5. getReportStatus returns status, fileUrl, errorMessage, generatedAt
6. API route serves cached PDF or generates on-demand
