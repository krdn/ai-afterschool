...
**현재 상태:** v3.0 Issue Management & Auto DevOps Pipeline 진행 중.
...
## Current Milestone: v3.0 Issue Management & Auto DevOps Pipeline

**Goal:** 앱 내부에서 이슈를 등록하고, GitHub Issue 생성 → 브랜치 생성 → 수정 작업 → 테스트 → 배포까지의 전체 DevOps 파이프라인을 자동화

**Target features:**
- 이슈 등록 UI (모달, 스크린샷 캡처, 카테고리 분류)
- GitHub Issues API 연동 (이슈 생성, 라벨, 이미지 업로드)
- 이슈 카테고리 기반 자동 브랜치 생성 (fix/, feat/, improve/)
- 런타임 에러 자동 수집 (Sentry 연동 → 자동 이슈 생성)
- 자동 배포 파이프라인 (PR 생성 → 테스트 → 머지 → 배포)
- 이슈 대시보드 (상태 트래킹, 필터링, 파이프라인 진행 현황)
...
**v3.0 이슈 관리 & 자동 DevOps 파이프라인:**
- [ ] 이슈 등록 UI (모달, 스크린샷, 카테고리)
- [ ] GitHub Issues API 연동
- [ ] 이슈 기반 자동 브랜치 생성
- [ ] 런타임 에러 자동 수집 → 이슈 생성
- [ ] 자동 배포 파이프라인 (PR → 테스트 → 머지 → 배포)
- [ ] 이슈 대시보드 (상태 트래킹)
